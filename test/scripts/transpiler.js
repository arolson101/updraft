    ...
    var load = function (url, idx) {
        ...
        xhr.onreadystatechange = function () {
            ...
            if (xhr.status === 0 || xhr.status === 200) {
                scripts.loaded++;
                scripts.data[idx] = xhr.responseText;
                scripts.name[idx] = url;
        ...
        if (window.sessionStorage && sessionStorage.getItem('typescript' + hashCode(scripts.data.join('')))) {
            ...
            (function () {
                ...
                for (num = 0; num < scripts.data.length; num++) {
                    ...
                    source += ts.transpile(src, undefined, filename);
                }
            })();
        }
        ...
        for (i = 0; i < script.length; i++) {
            if (script[i].type == 'text/typescript') {
                if (script[i].src) {
                    ...
                    scripts.data.push("");
                    scripts.name.push("");
                    load(script[i].src, scripts.data.length - 1);
                } else {
